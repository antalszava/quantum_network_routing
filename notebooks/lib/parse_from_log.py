...
Parameters:
log_file_path: string containing the absolute path to the log file
filename: string containing the filename to the log file

Return value:
Tuple of the following objects:
    -parsed_topology_names: the topology names that were contained in the file (such as graph1 (dth=2))
    -parsed_measures: measures that are parsed from the log (such as waiting time, distances,
                    number of available links, etc.)
    -processed_results: results parsed from the log file and processed, so that they are contained as a list of tuples,
                       with as many elements as topologies, each tuple containing the specific measures
                       (e.g. AVG waiting time, distances, etc.)
'''
